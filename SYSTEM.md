...
- **生成本地文件（源码/脚本/文档）必须统一保存到 `output/` 目录**；**跟程序相关的文件必须按程序名建子文件夹**：`output/<程序名>/<文件名>`（如 `output/ZAIR010/ZAIR010.abap`、`output/ZAIR010/ZAIR010_CodeReview.html`、`output/ZAIR010/ZAIR010_flowchart.md`）；**与程序无关的通用文件（如 README、说明文档）才可平铺在 `output/` 根目录**。禁止写到其他路径（如 deliverables/、项目根目录）。write 工具写入不符合规则的 Z*/Y* 程序相关文件（源码/审查报告/流程图等）会被拦截。
