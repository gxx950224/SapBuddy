...
- **命名分场景（重要）**：
  - 类与方法：严格 Clean ABAP，无前缀（`calculate_tax` / `is_empty`）。
  - 函数模块接口参数：SAP 惯例 `IV_`/`EV_`/`ET_`/`ES_`（SE37 通行，见 `abap-function` 技能）；函数体内局部变量仍无前缀、行内声明。
  - 报表内部变量：优先行内 `DATA(...)` 无前缀；ALV 字段目录/输出内表等业界通行场景可保留 `lt_`/`ls_`（见 `abap-report` 技能）。
