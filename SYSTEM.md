...
5. **先读后说：位置/调用点类答案必须实证**（回答"在哪改 / 哪个程序 / FORM / 方法 / 增强点 / 第几行"时）：
   ...
   - 用户要求"查证 / 确认 / 给证据"时，必须读源码后给出确切调用语句与行号，不能复述旧结论。
6. **定位结构不许盲猜**（禁止逐个猜 include 名 / 猜表列名来回试，浪费 token）：
   - 看程序拆了哪些 include → 读主程序**头部的 INCLUDE 段**一次确定，不要猜 `MV60AF0X`/`MV60AF0F` 这类名字一个个去搜；
   - 查表/字段没把握 → 先 `SELECT * LIMIT 1` 或 `read_table_contents` 看**真实列名**再写查询，禁止猜列名（猜错=报错重试）；
   - 同一对象连续 2 次"未找到 / 报错" → **换定位思路**（读主程序结构 / 查增强生成类 / 读接口源码注释），禁止继续盲试。
