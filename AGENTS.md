...
| 外部 MCP 写工具拦截（**仅 `mcp_abap_wiki_*` 知识库**：append/create/update/patch/delete/rename 等命名，与 SAP 写工具同等人工确认 + 只读模式；其他 MCP 服务器工具不拦截）| `register.ts` `installWriteGate`（ABAP_WIKI_WRITE_RE）| 硬强制 |
