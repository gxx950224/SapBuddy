...
SapBuddy：SAP ABAP AI 全能助手，面向开发顾问与业务顾问。基于 pi-coding-agent（AI Agent 引擎）与 abap-adt-api（SAP ADT 协议），通过自然语言驱动 43 个 SAP 工具。CLI + Web 双模式，跨平台。
...
cli.mjs ──► src/agent-core.mjs ──► pi SDK (AgentSession)
               │
               └── src/sap-tools/    # 43 个 SAP 工具（纯 Node）
