...
8. **生成 HTML 报告（需用户确认）。** 先把审查结论摘要展示给用户（问题数量/严重度分布/关键风险），**明确请求确认**后再生成文件：
   将 `assets/report-template.html` 复制到 `output/` 目录，
   把每个 `{{TOKEN}}` 占位符替换为真实内容（保留现有 CSS，不要改样式），
   保存为 `output/<程序名或函数名>_CodeReview.html`。
   ⚠️ 若 write 被拦截（提示"审查 HTML 报告需人工确认"）：把结论摘要发给用户，等用户输入"确认"后重试生成。
