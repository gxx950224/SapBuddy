...
> 命名规则、编码规范与开发流程一律以 `dev-write` 技能和 `AGENTS.md` 为准——**冲突时规范优先**。
> **字段类型**：本技能示例中的裸内置类型（`string`/`i`/`char1` 等）仅为语法演示——
> 实际项目里，自建表/结构字段必须使用 DDIC 数据元素（禁裸内置类型），找不到标准元素时创建 Z 数据元素 + 域；
> 程序内局部变量/参数允许裸类型（见 dev-write 技能字段类型）。
> 本技能示例中出现的 `SELECT *` 等写法仅为语法演示，SapBuddy 项目开发中禁止。
...
此技能包含 29 个综合参考文件，涵盖 ABAP 开发的所有方面：
