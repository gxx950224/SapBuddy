...
用自然语言驱动 **43 个 SAP 工具**：搜索、读码、开发、审查、ATC、单测、SQL、DDIC 管理、程序解读……
...
- 🛠 **43 个内置 SAP 工具**：对象搜索 / 源码读取 / where-used / ATC 质量门禁 / 单元测试 / SQL 查询 / 传输请求 / 版本历史 / ST22 dump 分析 / DDIC 管理（表/结构/数据元素/域/CDS）/ 文本元素与多语言翻译 / 调试辅助
...
![Web 对话主界面](docs/images/Web 对话主界面.png)

**一次真实对话的成果** —— 用一句大白话让 AI 读取并分析 SAP 对象，直接给出结果

![一次真实对话的成果](docs/images/一次真实对话的成果.png)

**代码审查报告** —— 审查程序自动生成 4 页签 HTML 报告（质量评分 / 规范问题 / ATC）

![代码审查报告](docs/images/代码审查报告.png)

**CLI 终端** —— 命令行全屏交互模式，终端里直接对话

![CLI 终端](docs/images/CLI终端.png)
...
sapbuddy tools                     # 列出 43 个 SAP 工具 + 已配置的 MCP 工具
```
...
- **直接集成**：43 个工具是纯函数模块，直接注册为 Agent customTools，不依赖 MCP 框架
...
│   ├── sap-tools/          # 43 个 SAP 工具（TypeScript，基于 abap-adt-api）
...
- [x] 43 个 SAP 工具（搜索/读码/写/ATC/单测/SQL/DDIC/文本/翻译/调试）
- [x] CLI + Web 双模式
- [x] 开发客户端守卫 + 写前审核门禁
- [x] MCP 服务器接入
- [ ] 批量代码审查报告（多对象）
