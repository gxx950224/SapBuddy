...
用自然语言驱动 **44 个 SAP 工具**：搜索、读码、开发、审查、ATC、单测、SQL、DDIC 管理、程序解读……
...
- 🛠 **44 个内置 SAP 工具**：对象搜索 / 源码读取 / where-used / ATC 质量门禁 / 单元测试 / SQL 查询 / 传输请求 / 版本历史 / ST22 dump 分析 / DDIC 管理（表/结构/数据元素/域/CDS）/ 文本元素与多语言翻译 / 调试辅助
...
sapbuddy tools                     # 列出 44 个 SAP 工具 + 已配置的 MCP 工具
```
...
- **直接集成**：44 个工具是纯函数模块，直接注册为 Agent customTools，不依赖 MCP 框架
...
│   ├── sap-tools/          # 44 个 SAP 工具（TypeScript，基于 abap-adt-api）
...
- [x] 44 个 SAP 工具（搜索/读码/写/ATC/单测/SQL/DDIC/文本/翻译/调试）
- [x] CLI + Web 双模式
- [x] 开发客户端守卫 + 写前审核门禁
- [x] MCP 服务器接入
- [ ] 批量代码审查报告（多对象）
